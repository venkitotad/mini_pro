...
const errorCodes = require('../../shared/better-auth.BM1EYiYB.cjs');
...
const cloudflareTurnstile = async ({
  ...
  if (!response.data || response.error) {
    throw new Error(errorCodes.INTERNAL_ERROR_CODES.SERVICE_UNAVAILABLE);
  }
  if (!response.data.success) {
    return middlewareResponse({
      message: errorCodes.EXTERNAL_ERROR_CODES.VERIFICATION_FAILED,
      status: 403
    });
  }
  return void 0;
  ...
  onRequest: async (request, ctx) => {
    try {
      ...
      if (!options.secretKey) {
        throw new Error(errorCodes.INTERNAL_ERROR_CODES.MISSING_SECRET_KEY);
      }
      const captchaResponse = request.headers.get("x-captcha-response");
      const remoteUserIP = request.headers.get("x-captcha-user-remote-ip") ?? void 0;
      if (!captchaResponse) {
        return middlewareResponse({
          message: errorCodes.EXTERNAL_ERROR_CODES.MISSING_RESPONSE,
          status: 400
        });
      }
      ...
      return middlewareResponse({
        message: errorCodes.EXTERNAL_ERROR_CODES.UNKNOWN_ERROR,
        status: 500
      });
    }
  }
